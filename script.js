// Map init — default Roma, poi aggiornato con posizione reale
...
// Chiedi subito la posizione per centrare la mappa
document.getElementById('gpsInfo').innerText = 'Ricerca posizione...';
if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
        pos => {
            map.setView([pos.coords.latitude, pos.coords.longitude], 15);
            document.getElementById('gpsInfo').innerText = `Posizione trovata · Precisione ±${Math.round(pos.coords.accuracy)}m`;
        },
        err => {
            const msgs = { 1: 'Permesso GPS negato — mappa su Roma', 2: 'GPS non disponibile', 3: 'Timeout GPS' };
            document.getElementById('gpsInfo').innerText = msgs[err.code] || 'GPS non disponibile';
        },
        { enableHighAccuracy: true, timeout: 8000 }
    );
} else {
    document.getElementById('gpsInfo').innerText = 'GPS non supportato dal browser';
}
